...
public class ProverCNIMethod {
	public static  <T extends ProverMethod> ProofResult prove(Prover prover, Function<CommonData, T> methodFactory) {
		...
		commonData.prover=prover;
		commonData.allPredecessorPoints=allPredecessorPoints;
		commonData.statement=statement;
		commonData.kernel=kernel;
		commonData.loc=loc;
		commonData.predefinitions=predefinitions;
		commonData.predefs=predefs;
		commonData.primeLabels = primeLabels;

		T methodInstance = methodFactory.apply(commonData);
