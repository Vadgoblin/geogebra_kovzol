...
public class ProverCNIMethodReal implements ProverMethod {
	private static class CNIDefinition {
	...
	ArrayList<String> toEliminateLhsPrimed;
	ArrayList<String> toEliminateRhsVars;


	public ProverCNIMethodReal(CommonData data) {
		this.data = data;
	}
	...
	public Prover.ProofResult execute() {
		...
		if (data.prover.getShowproof()) {
			data.prover.addProofLine(data.loc.getMenuDefault("TheHypotheses", "The hypotheses:"));
		}
		for (GeoPoint ge : data.allPredecessorPoints) {
			if (ge.getParentAlgorithm() == null) {
				...
				try {
					...
					Log.debug(
							"The CNI method does not yet fully implement " + ge.getParentAlgorithm()
									.toString()
									+ " which is required for " + ge.getLabelSimple());
					return Prover.ProofResult.UNKNOWN;
				}
				if (def == null) {
					Log.debug("The CNI method does not yet implement " + ge.getParentAlgorithm()
							.toString()
							+ " which is required for " + ge.getLabelSimple());
					return Prover.ProofResult.UNKNOWN;
				}
				...
				if (data.prover.getShowproof()) {
					data.prover.addProofLine(data.loc.getPlain("ConsideringDefinitionA",
							ge.getLabelSimple() + " = "
									+ ge.getDefinition(
				...
				if (def.declaration != null) {
					...
					if (data.prover.getShowproof()) {
						data.prover.addProofLine(CmdShowProof.TEXT_EQUATION, def.declaration);
						...
						data.prover.addProofLine(CmdShowProof.EQUATION,
								addPrimesToLabels(def.declaration, data.primeLabels));
					}
				}
				if (def.zeroRelation != null) {
					...
					if (data.prover.getShowproof()) {
						data.prover.addProofLine(CmdShowProof.TEXT_EQUATION,
								def.zeroRelation + "=0");
					}
				}
				...
				if (def.realRelation != null) {
					...
					for (String CASrealRelation : CASrealRelations) {
						realRelationsNo++;
						String expression =
								CASrealRelation + "=" + VARIABLE_R_STRING
										+ realRelationsNo;
						realRelations += expression + ",";
						if (data.prover.getShowproof()) {
							String rewriteProgram =
									"[" + data.predefs + expression + "][" + data.predefinitions.length + "]";
							String expression2 = executeGiac(rewriteProgram);
							data.prover.addProofLine(CmdShowProof.TEXT_EQUATION,
									lhs(expression) + "=" + expression2
											+ Unicode.IS_ELEMENT_OF + "\u211D");

							if (!algebraicRelationExplained) {
								data.prover.addProofLine(
										data.loc.getPlainDefault("CNIAlgebraicRelations",
							...
							String lhs2 = addPrimesToLabels(lhsProgram, data.primeLabels);
					...
					if (def.warning == WARNING_PERPENDICULAR_OR_PARALLEL) {
						data.prover.addProofLine(CmdShowProof.PROBLEM,
								data.loc.getMenuDefault("PerpendicularityParallelism",
										"Perpendicularity means perpendicularity or parallelism simultaneously."));
					}
					if (def.warning == WARNING_EQUALITY_OR_COLLINEAR) {
						data.prover.addProofLine(CmdShowProof.PROBLEM,
								data.loc.getMenuDefault("EqualityCollinearity",
		...
		CNIDefinition def = null;
		try {
			def = getCNIThesisDefinition(data.statement);
		} catch (Exception e) {
			Log.debug(
					"The CNI method does not yet fully implement "
							+ data.statement.getParentAlgorithm()
							.toString());
			return Prover.ProofResult.UNKNOWN;
		}
		if (def == null) {
			Log.debug("The CNI method does not yet implement " + data.statement.getParentAlgorithm()
					.toString());
			return Prover.ProofResult.UNKNOWN;
		}
		if (data.prover.getShowproof()) {
			data.prover.addProofLine(data.loc.getMenuDefault("TheThesis", "The thesis:"));
			data.prover.addProofLine(
					data.statement.getParentAlgorithm()
		...
		if (def.realRelation != null) {
			...
			for (int i = 0; i < nrRels - 1; i++) {
				...
				if (data.prover.getShowproof()) {
					...
					data.prover.addProofLine(CmdShowProof.EQUATION, rk + PRIME + ":=" + lhs2);
				}
			}
			String thesis = CASrealRelations[nrRels - 1] + "=" + VARIABLE_R_STRING;
			realRelations += thesis;
			if (data.prover.getShowproof()) {
				String rewriteProgram = "[" + data.predefs + thesis + "][" + data.predefinitions.length + "]";
				String thesis2 = executeGiac(rewriteProgram);
				data.prover.addProofLine(CmdShowProof.TEXT_EQUATION, lhs(thesis) + "=" + thesis2);
				...
				data.prover.addProofLine(data.loc.getPlainDefault(
				...
				if (def.warning == WARNING_ANGLE) {
					data.prover.addProofLine(CmdShowProof.PROBLEM,
							data.loc.getMenuDefault("AngleAmbiguity",
		...
		// Specialization.
		if (data.prover.getShowproof()) {
			data.prover.addProofLine(CmdShowProof.SPECIALIZATION,
					data.loc.getMenuDefault("WlogCoordinates",
		...
		for (GeoElement ge : freePoints) {
			if (i == 0 && maxSpecRestriction < 2) {
				...
				if (data.prover.getShowproof() && data.prover.getShowEliminate()) {
					specEqList.add(getUniqueLabel(ge) + PRIME + "=0");
				}
				specCode += spec1 + "\n";
				if (data.prover.getShowproof()) {
					data.prover.addProofLine(CmdShowProof.TEXT_EQUATION, spec1);
				}
				specialized.add(ge);
			}
			if (i == 1 && maxSpecRestriction < 1) {
				...
				specCode += spec2 + "\n";
				if (data.prover.getShowproof()) {
					data.prover.addProofLine(CmdShowProof.TEXT_EQUATION, spec2);
				}
				specialized.add(ge);
			}
			i++;
		...
		rest += ")]";
		int codeLengthLines = data.predefinitions.length + declarationsA.length + 1;
		...
		if (data.prover.getShowproof()) {
			data.prover.addProofLine(data.loc.getMenuDefault("EliminateAllFreeVariables",
					"We eliminate all variables that correspond to free points."));

			if (data.prover.getShowEliminate()) {

				data.prover.addProofLine(data.loc.getMenuDefault("CNIEliminateCommandInfo",
				...
				data.prover.addProofLine(CmdShowProof.EQUATION, ggbEliminateCommand);
			}
		...
		if (elimIdeal.equals("{{}}")) {
			...
			if (data.prover.getShowproof()) {
				data.prover.addProofLine(CmdShowProof.PROBLEM,
						data.loc.getMenuDefault("NoCorrespondenceBetweenHypothesesThesis",
		...
		program = "[[" + VARIABLE_I_STRING + ":= " + elimIdealL + "],[deg:=inf],[degi:=0],"
				+ "[for (k:=0;k<size(" + VARIABLE_I_STRING + ");k++) { d:=degree("
				+ VARIABLE_I_STRING
				+ "[k]," + VARIABLE_R_STRING + ");"
		...
		if (minDegreeA[0].equals("+infinity")) {
			...
			if (data.prover.getShowproof()) {
				data.prover.addProofLine(CmdShowProof.PROBLEM,
						data.loc.getMenuDefault("ThesisCannotBeExpressed",
		...
		if (minDegreeI == 1) {
			...
			if (data.prover.getShowproof()) {
				data.prover.addProofLine(
						data.loc.getPlainDefault("ThesisACanBeExpressedAsRationalBecauseALinear",
								"The thesis (%0) can be expressed as a rational expression of the hypotheses, because %0 is"
										+ " linear in the following polynomial equation:",
								VARIABLE_R_STRING));
				data.prover.addProofLine(minDegreeA[1] + "=0");
				...
				data.prover.addProofLine(data.loc.getMenuDefault("CNISimplifyBoth",
						"We now simplify both expressions. This makes them easier to compare:"));

				data.prover.addProofLine(CmdShowProof.EQUATION,
						VARIABLE_R_STRING + PRIME + PRIME + ":=" + rExprPrimed);
				data.prover.addProofLine(CmdShowProof.EQUATION, "Simplify(" + rExprPrimed + ")");
				...
				if (isNumericConstant(simplifiedRExpr)) {
					if (simplifiedThesis != null && simplifiedRExpr.equals(simplifiedThesis)) {
						data.prover.addProofLine(data.loc.getMenuDefault("CNISimplifiedSameNumber",
								"Both simplified expressions are the same number. So the result matches the thesis."));
					} else {
						data.prover.addProofLine(data.loc.getMenuDefault("CNISimplifiedToNumber",
					...
					data.prover.addProofLine(data.loc.getMenuDefault("CNISimplifiedEqualThesis",
			...
			if (divVars.equals("{}")) {
				if (rMustBeZero) {
					if (minDegreeA[1].equals(VARIABLE_R_STRING) ||
						...
						if (data.prover.getShowproof()) {
							data.prover.addProofLine(CmdShowProof.CONCLUSION,
									data.loc.getMenuDefault("ThesisZeroStatementTrue",
					...
					if (data.prover.getShowproof()) {
						data.prover.addProofLine(CmdShowProof.PROBLEM,
								data.loc.getMenuDefault("ThesisShouldBeZero",
				...
				if (data.prover.getShowproof()) {
					data.prover.addProofLine(
							data.loc.getMenuDefault("ThesisCanBeExpressedPolynomial",
									"The thesis can be expressed as a polynomial expression of the hypotheses."));
					data.prover.addProofLine(CmdShowProof.CONCLUSION,
							data.loc.getMenuDefault("HypothesesRealThesisReal",
			...
			if (data.prover.getShowproof()) {
				data.prover.addProofLine(
						data.loc.getPlainDefault("SolvingForARequiresDivByB",
								"Solving for %0 requires a division by %1.",
								new String[]{VARIABLE_R_STRING, divisor}));
				data.prover.addProofLine(data.loc.getMenuDefault("AssumeDivisorZero",
			...
			if (data.prover.getShowproof() && data.prover.getShowEliminate()) {
				data.prover.addProofLine(data.loc.getMenuDefault("CNIEliminateCommandInfoDivisor",
			...
			if (elimIdeal2.equals("{{1}}")) {
				...
				if (data.prover.getShowproof()) {
					data.prover.addProofLine(data.loc.getMenuDefault("DivisorCannotBeZero",
			...
			if (minDegree2A[0].equals("+infinity")) {
				...
				if (data.prover.getShowproof()) {
					data.prover.addProofLine(CmdShowProof.PROBLEM,
							data.loc.getMenuDefault("AssumingZeroThesisCannotBeExpressed",
			...
			if (minDegree2I == 1) {
				...
				if (data.prover.getShowproof()) {
					data.prover.addProofLine(
							data.loc.getPlainDefault(
									"ThesisACanBeExpressedNowAsRationalBecauseALinear",
									"The thesis (%0) can now be expressed as a rational expression of the hypotheses, because %0 is"
											+ " linear in the following polynomial equation:",
									VARIABLE_R_STRING));
					data.prover.addProofLine(minDegree2A[1] + "=0");
					...
					data.prover.addProofLine(CmdShowProof.EQUATION,
							VARIABLE_R_STRING + PRIME + PRIME + ":=" + rExpr2Primed);
					data.prover.addProofLine(CmdShowProof.EQUATION,
							"Simplify(" + rExpr2Primed + ")");
					...
					if (isNumericConstant(simplifiedRExpr2)) {
						if (simplifiedThesis2 != null && simplifiedRExpr2.equals(
								simplifiedThesis2)) {
							data.prover.addProofLine(
									data.loc.getMenuDefault("CNISimplifiedSameNumber",
											"Both simplified expressions are the same number. So the result matches the thesis."));
						} else {
							data.prover.addProofLine(
									data.loc.getMenuDefault("CNISimplifiedToNumber",
											"The expression simplifies to a fixed number. This means the hypotheses already determine its value."));
						}
					} else if (thesisDefinitionPrimed != null) {
						data.prover.addProofLine(data.loc.getMenuDefault("CNISimplifiedEqualThesis",
				...
				Log.debug("The second elimination ideal contains " + minDegree2A[1]
						+ ", it is linear in r_.");
				// Check if r can be expressed without a division:
				// lvar(coeff(2*r_+1,r_)[0])
				...
				if (divVars2.equals("{}")) {
					if (rMustBeZero) {
						if (minDegree2A[1].equals(VARIABLE_R_STRING) || minDegree2A[1].equals(
								"-" + VARIABLE_R_STRING)) {
							if (data.prover.getShowproof()) {
								data.prover.addProofLine(CmdShowProof.CONCLUSION,
										data.loc.getMenuDefault("ThesisZeroStatementTrue",
						...
						if (data.prover.getShowproof()) {
							data.prover.addProofLine(CmdShowProof.PROBLEM,
									data.loc.getMenuDefault("ThesisShouldBeZeroNow",
					...
					if (data.prover.getShowproof()) {
						data.prover.addProofLine(
								data.loc.getMenuDefault("NowThesisCanBeExpressedPolynomial",
				...
				if (data.prover.getShowproof()) {
					data.prover.addProofLine(CmdShowProof.PROBLEM,
							data.loc.getMenuDefault("ThesisStillContainsDivision",
			...
			if (data.prover.getShowproof()) {
				data.prover.addProofLine(CmdShowProof.PROBLEM,
						data.loc.getMenuDefault("ThesisCannotBeExpressedDivision",
		...
		if (minDegreeI == 2) {
			...
			program = "[[D:=discriminant(" + minDegreeA[1] + "," + VARIABLE_R_STRING
					+ ")],[total_degree(D,lvar(D))]][1]";
	...
	CNIDefinition getCNIHypothesisDefinition(GeoElement ge) {
		...
		if (ae instanceof AlgoRotatePoint) {
			...
			if (P instanceof GeoPoint && a instanceof GeoAngle && C instanceof GeoPoint) {
				...
				long gcd = data.kernel.gcd(angleValDeg, 360);
	...
	CNIDefinition getCNIThesisDefinition(GeoElement ge) {
		...
		if (ae instanceof AlgoDependentBoolean) {
			...
			if (!en.getLeft().isGeoElement() || !en.getRight().isGeoElement()) {
				...
				if (en.getOperation() == Operation.EQUAL_BOOLEAN &&
					...
					GeoAngle a1 = (GeoAngle) ((ExpressionNode) en.getLeft()).getRightTree()
							.getSingleGeoElement();
					GeoAngle a2 =
							(GeoAngle) ((ExpressionNode) en.getRightTree()).getSingleGeoElement();
				...
				if (en.getOperation() == Operation.EQUAL_BOOLEAN &&
					...
					long gcd = data.kernel.gcd(angleValDeg, 180);
			...
			if (o == Operation.PARALLEL) {
				...
				if (ge1 instanceof GeoPoint && ge2 instanceof GeoConic
						&& ((GeoConic) ge2).isCircle()) {
	...
	private String executeGiac(String command) {
		GeoGebraCAS cas = (GeoGebraCAS) data.kernel.getGeoGebraCAS();
	...
	static String eqanglemul(GeoElement ge1, GeoElement ge2, GeoElement ge3, GeoElement ge4,
		...
		String ge5l = getUniqueLabel(ge5);
		String ge6l = getUniqueLabel(ge6);
		return "eqanglemul(" + ge1l + "," + ge2l + "," + ge3l + "," + ge4l + "," + ge5l + "," + ge6l
				+ "," + n + ")";
	}
